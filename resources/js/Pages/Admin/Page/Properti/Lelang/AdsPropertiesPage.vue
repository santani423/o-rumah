                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="flex flex-col col-span-2">
                                ...
                                <p class="text-sm text-gray-500">
                                    Jumlah karakter: {{ description.length }} /
                                    2000
                                </p>
                            </div>
                        </div>
...
const submit = () => {
    ...
    axios
        .post(route("admin.nav.lelang.store"), formData, {
            headers: {
                "Content-Type": "multipart/form-data",
            },
        })
        .then((response) => {
            console.log("Response from Laravel:", response.data);
            window.location.href = route("admin.nav.lelang");
        })
        .catch((error) => {
            console.error("Error submitting form:", error);
        });
...
// Menggunakan watcher untuk memantau perubahan pada textarea description
const checkDescriptionLength = () => {
    if (description.value.length > 2000) {
        description.value = description.value.slice(0, 2000); // Limit input to 2000 characters
    }

    const formatText = (style) => {
        document.execCommand(style, false, null); // Apply text formatting
    };
};
</script>
