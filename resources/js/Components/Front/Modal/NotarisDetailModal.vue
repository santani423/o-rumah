            ...
            <div class="fixed inset-0 overflow-y-auto">
                <div
                    ...
                    <TransitionChild as="template" enter="duration-300 ease-out"
                        ...
                        <DialogPanel
                            class="w-full overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl max-w-md">

                            <div class="flex border border-b items-center p-4">
                                <DialogTitle as="h3"
                            ...
                            <div class="p-6 flex flex-col gap-4">
                                <div class="flex items-center space-x-5">
                                    <div class="flex-shrink-0">
                                        <img class="w-20 h-20 rounded-full"
                                             :src="image" :alt="notaris.name"/>
                                    </div>
                                    <div class="flex-1 min-w-0 text-left">
                                        <a href="#" @click="setIsOpen(true)"
                                ...
                                <div v-if="notaris.company_name"
                                     class="flex-shrink-0">
                                    <div
                                        class="flex items-center px-4 py-3 space-x-4 bg-neutral-50 rounded-xl">
                                        <img
                                            class="h-10 w-10 object-cover aspect-square rounded-full"
                                            :src="notaris.company_image"
                                            :alt="notaris.company_name"/>
                                        <p class="flex-1 text-sm truncate">
                                            {{ notaris.company_name }}</p>
                                    </div>
                                </div>
                                <div
                                    class="flex flex-col gap-2">
                                    <button v-if="notaris.phone" type="button"
                                            @click.prevent="handlePhone(`tel:${notaris.phone}`)"
                                        ...
                                        <PhoneIcon class="-ml-0.5 h-5 w-5"
                                                   aria-hidden="true"/>
                                        Telepon
                                    </button>
                                    <button v-if="notaris.wa_phone" type="button"
                                            @click.prevent="handlePhone(`https://wa.me/${notaris.wa_phone}`)"
...
// Define props
const { isOpen, notaris } = defineProps({
    isOpen: Boolean,
    notaris: Object,
})
...
const image = computed(() => {
    return notaris.image ? notaris.image : 'https://via.placeholder.com/100'
})
